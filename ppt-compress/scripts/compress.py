...
def compress_images(media_dir):
    """压缩媒体目录中的大图片"""
    if os.name == 'nt':
        print(f"⚠️  Windows 環境下跳過 sips 圖片壓縮")
        return
...
def convert_to_pdf(pptx_path, output_dir):
    ...
    if not soffice:
        print(f"❌ 未找到LibreOffice，請先安裝: https://zh-tw.libreoffice.org/download/")
        return None
...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        compressed_pptx = os.path.join(output_dir, 
            os.path.splitext(os.path.basename(pptx_path))[0] + '_compressed.pptx')
        repack_pptx(extract_dir, compressed_pptx)
        
        # 步骤4: 转换为PDF
        print(f"DEBUG: 正在轉換 {compressed_pptx} 到 {output_dir}")
